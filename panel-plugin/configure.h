...
static void moc_configure_create(XfcePanelPlugin *plugin, struct MocConfig *config) {
    GtkWidget *dlg = xfce_titled_dialog_new();
    GtkWidget *chooser, *label, *entry;

    gtk_window_set_title(GTK_WINDOW(dlg), _("Настройки"));
    ...
    chooser = gtk_file_chooser_button_new (_("Выбор папки"), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
    ...
    new_label (GTK_GRID (grid), 1, _("Папка с музыкой:"), chooser);
    ...
    new_label (GTK_GRID (grid), 3, _("Команда запуска MOC:"), entry);
...
static void moc_show_about(XfcePanelPlugin *plugin, struct MocConfig *config) {
    gtk_show_about_dialog (NULL,
      "logo-icon-name", "xfce.plugin.moc.icon",
      "license", xfce_get_license_text (XFCE_LICENSE_TEXT_LGPL),
      "version", PACKAGE_VERSION,
      "program-name", PACKAGE_NAME,
      "comments", _("Xfce плагин для консольного аудиоплеера MOC"),
